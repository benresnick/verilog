...
module sysarr_tb;
    ...
    logic signed [DATA_WIDTH-1:0] activation_in [0:1];
    logic                         activation_valid_in [0:1];

    // Weights enter from the top of each column
    logic signed [DATA_WIDTH-1:0] weight_in [0:1];
    logic                         weight_valid_in [0:1];
    ...
    logic signed [ACC_WIDTH-1:0] partial_sum [0:1][0:1];
    ...
    task automatic drive_cycle(
        ...
        begin
            ...
            activation_in[0]       = act_row0;
            activation_valid_in[0] = act_row0_valid;

            activation_in[1]       = act_row1;
            activation_valid_in[1] = act_row1_valid;

            weight_in[0]       = wgt_col0;
            weight_valid_in[0] = wgt_col0_valid;

            weight_in[1]       = wgt_col1;
            weight_valid_in[1] = wgt_col1_valid;
    ...
    task automatic check_partial_sums(
        ...
        begin
            ...
            if ($signed(partial_sum[0][0]) !== expected_00) begin
                $error(
                    "%s: C00 incorrect. Expected %0d, got %0d",
                    test_name,
                    expected_00,
                    $signed(partial_sum[0][0])
                );
                error_count++;
            end

            if ($signed(partial_sum[0][1]) !== expected_01) begin
                $error(
                    "%s: C01 incorrect. Expected %0d, got %0d",
                    test_name,
                    expected_01,
                    $signed(partial_sum[0][1])
                );
                error_count++;
            end

            if ($signed(partial_sum[1][0]) !== expected_10) begin
                $error(
                    "%s: C10 incorrect. Expected %0d, got %0d",
                    test_name,
                    expected_10,
                    $signed(partial_sum[1][0])
                );
                error_count++;
            end

            if ($signed(partial_sum[1][1]) !== expected_11) begin
                $error(
                    "%s: C11 incorrect. Expected %0d, got %0d",
                    test_name,
                    expected_11,
                    $signed(partial_sum[1][1])
                );
                error_count++;
            end
    ...
    initial begin
        ...
        activation_in[0]       = '0;
        activation_valid_in[0] = 1'b0;

        activation_in[1]       = '0;
        activation_valid_in[1] = 1'b0;

        weight_in[0]       = '0;
        weight_valid_in[0] = 1'b0;

        weight_in[1]       = '0;
        weight_valid_in[1] = 1'b0;
        ...
        if (dut.activation_valid[0][1] !== 1'b0) begin
            $error("Reset failed to clear act_valid_00_to_01");
            error_count++;
        end

        if (dut.activation_valid[1][1] !== 1'b0) begin
            $error("Reset failed to clear act_valid_10_to_11");
            error_count++;
        end

        if (dut.weight_valid[1][0] !== 1'b0) begin
            $error("Reset failed to clear weight_valid_00_to_10");
            error_count++;
        end

        if (dut.weight_valid[1][1] !== 1'b0) begin
        ...
        if ($signed(dut.activation[0][1]) !== 1 ||
            dut.activation_valid[0][1] !== 1'b1) begin
        ...
        if ($signed(dut.weight[1][0]) !== 5 ||
            dut.weight_valid[1][0] !== 1'b1) begin
        ...
        if ($signed(dut.activation[0][1]) !== 2 ||
            dut.activation_valid[0][1] !== 1'b1) begin
        ...
        if ($signed(dut.activation[1][1]) !== 3 ||
            dut.activation_valid[1][1] !== 1'b1) begin
        ...
        if ($signed(dut.weight[1][1]) !== 6 ||
            dut.weight_valid[1][1] !== 1'b1) begin
    ...
    initial begin
        ...
        $dumpfile("sysarr_tb.vcd");
        $dumpvars(0, sysarr_tb);
